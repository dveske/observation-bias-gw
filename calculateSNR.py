...
for ifo in ('H1','L1','V1'): #compute SNR generated in each detector separately
    ...
    for m2 in range (5,100): #Consider small mass between [5,99] solar masses
	    ...
	    for m1 in range (m2,min(100,10*m2+1)): #Consider heavy mass between [m2,min(99,10*m2)] solar masses. Mass ratios >10 are not applicable
	        ...
	        t,h,dyn=sur(m1/m2,[0,0,0],[0,0,0],dt=d,f_low=25,units='mks',dist_mpc=1000,M=m1+m2,inclination=0,phi_ref=0) #the gw waveform starting at 25 Hz for an event with specified masses, located 1 Gpc away, face on orientation, no effect of redshift, no spin
	        ...
	        snr[m1][m2]=2*((numpy.sum((numpy.fft.fft(h.real))*(numpy.fft.fft(h.real)).conjugate()/newft)/len(t)*(d))).real #compute SNR for plus polarization. Factor of 2 is for correcting the one sided PSD. For 0 inclination, SNR is same for + and x polarizations.
